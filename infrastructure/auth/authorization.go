...
func (r *authRepository) CanStartExam(ctx context.Context, userId, examId string) (int64, error) {
...
func (r *authRepository) CanAccessExam(ctx context.Context, ginCtx *gin.Context, userId, examId string) error {
	inClass, err := r.IsUserInExamClass(ctx, userId, examId)
	if err != nil {
		return err
	}
...
func (r *authRepository) CanAccessProblem(ctx context.Context, ginCtx *gin.Context, userId, problemId string) error {
	user, err := r.GetUserById(ctx, userId)
	if err != nil {
		return err
	}
	fmt.Println("ini user:", user.RoleID)
	...
	if err := r.ValidateSEBRequest(ginCtx, ctx, examSession.ExamID); err != nil {
		return fmt.Errorf("SEB validation failed: %w", err)
	}
...
func (r *authRepository) ValidateSEBRequest(GinCtx *gin.Context, ctx context.Context, examId string) error {
	var exam entity.Exam
